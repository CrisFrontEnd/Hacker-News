...
const allNews = document.querySelector('.news-available') // Questa variabile raccoglierà tutte le notizie
const loadNews = document.getElementById('load-button') // Questa variabile caricherà altre 10 notizie cliccando il bottone




async function getData() { // Per prima cosa creo la funzione che carica i dati dall'URL
    let newsElement = await axios.get(url) // Attraverso il client Axios, e il suo metodo ".get" effettuo la chiamata all'API
    let newsStart = 0 // Imposto il valore iniziale delle news pari a 0
    createElement(newsElement.data, newsStart) // Gli Elementi "newsElement" e "newsStart" per una lista di eventi futuri.
    ...
    loadNews.addEventListener('click', () => { // Creo la parte di codice che permette al bottone di caricare le altre 10 news
        ...
        createElement(newsElement.data, newsStart) // Questi sono gli stessi elementi che si creano quando si avvia l'applicazione
    })
}

function createElement(arr, listNews) { // Creo la funzione che raccoglie i dati
    Promise.all(arr.slice(listNews, listNews + 10).map((item) => // Richiamo il metodo ""Promise.all" che restituisce un singola promise solo se tutte le promise dell'input vengono soddisfatte
    ...
    .then(( res => {  // Uso il metodo ".then()" che ritorna una promise
      res.forEach(news => {
        allNews.insertAdjacentHTML("beforeend", /* Qui invece creo dinamicamente la classe che conterrà le news; In particolare, da notare l'uso di "_.get()", metodo proveniente dalla libreria di Lodash, per mezzo della quale si ricava l'url della news e il tempo passato dalla pubblicazione. */
                               `<div class ="news"> 
                                  <h3 class="textnews"><a href=${_.get(news, "data.url", location.href)} target =_blank>
    ...
    function timeDifference(timeNow, timeNews) { // Questa funzione che calcola il tempo esatto dalla pubblicazione della news
